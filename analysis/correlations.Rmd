...
for (marker.type in unique(markers[,2])){
  ...
  sce_naive <- runTSNE(sce_naive, exprs_values="norm_exprs", feature_set = marker.set, perplexity=30)
...
for (marker.type in unique(markers[,2])){
  ...
  sce_primed <- runTSNE(sce_primed, exprs_values="norm_exprs", feature_set = marker.set, perplexity=30)
