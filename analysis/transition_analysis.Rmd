...
all.markers_naive <- findMarkers(sce_naive, clusters=sce_naive$cluster, design=blocking)
clust.num_naive <- "2"
...
top.markers_naive <- rownames(marker.set_naive)[which(marker.set_naive$Top <= 100)]
...
library(pheatmap)
clust.col_naive <- topo.colors(3)

pheatmap(heat.vals_naive, 
    annotation_col=data.frame(Cluster=as.character(sce_naive$cluster), row.names=colnames(heat.vals_naive)), 
    show_rownames = FALSE, show_colnames = FALSE,
    annotation_colors=list(Cluster=setNames(clust.col_naive, unique(sce_naive$cluster))))
```
...
Finally, we perform a holms-corrected unpaired t.test to test whether the differences in gene expression between the transition and the other populations,respectively, are more pronounce between transition/primed than transition/naive.
...
pops <- data.frame(Cell=colnames(sce), Type=all.pheno)
