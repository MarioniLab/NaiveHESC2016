...
stopifnot(identical(rownames(naive_1), rownames(naive_2)))
stopifnot(identical(rownames(naive_1), rownames(primed_1)))
stopifnot(identical(rownames(naive_1), rownames(primed_2)))
var.tab <- cbind(naive_1$bio, naive_2$bio, primed_1$bio, primed_2$bio)
rownames(var.tab) <- rownames(naive_1)
head(var.tab)
```
...
There is no evidence for DVG between conditions in the entire set of genes, though maybe we're just underpowered with only two replicates.
