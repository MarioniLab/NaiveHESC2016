...
lfc <- var.out_primed$mean - var.out_naive$mean
dbio <- var.out_primed$bio - var.out_naive$bio
plot(lfc, dbio, xlab=expression(Log[2]~"fold change"), 
    ylab=expression(Delta*"biological component"), pch=16)
phvg <- match(rownames(hvg.out_primed), rownames(var.out_primed))
nhvg <- match(rownames(hvg.out_naive), rownames(var.out_naive))
all.hvgs <- c(nhvg, phvg)
cols <- rep(c("orange", "dodgerblue"), c(length(nhvg), length(phvg)))
set.seed(1000)
shuffled <- sample(length(cols))
all.hvgs <- all.hvgs[shuffled]
cols <- cols[shuffled]
points(lfc[all.hvgs], dbio[all.hvgs], col=cols, pch=16)
in.both <- intersect(phvg, nhvg)
points(lfc[in.both], dbio[in.both], col="salmon", pch=16)
```
