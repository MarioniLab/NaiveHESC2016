...
If simply the reduced count size were driving the increase in variability, we should see differences in the technical variance.
This is not the case - or, at least, not enough to explain the differences in the biological components.

```{r}
plot(var.out_naive$mean, var.out_naive$tech, pch=16, col="orange", 
    xlab="Log-mean", ylab="Technical variance")
points(var.out_primed$mean, var.out_primed$tech, pch=16, col="dodgerblue")
legend("topright", col=c("dodgerblue", "orange"), legend=c("Primed", "Naive"), pch=16)
```
