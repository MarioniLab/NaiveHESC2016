---
title: "Single-cell RNA-seq of naive/primed embryonic stem cells: naive-only correlations"
...
We specifically use only the naive cells here, excluding the previously identified 'transition' subpopulation.

```{r naivein}
library(scran)
sce_naive <- readRDS("~/Desktop/analysis/sce_naive.rds")
dim(sce_naive)
true.groups <- read.table("results-naive/groups.tsv", header=TRUE, stringsAsFactors=FALSE)
sce_naive <- sce_naive[,true.groups$Type[match(colnames(sce_naive), true.groups$Cell)]!="transition"]
dim(sce_naive)
...
plotPCA(sce_naive, exprs_values="norm_exprs", colour_by = "CDK1") + fontsize
plotTSNE(sce_naive, exprs_values="norm_exprs", colour_by='CDK1',
    perplexity=5, rand_seed=100) + fontsize
```

Then we repeat the same using only the subset of chosen genes of interest. 

```{r naive.pca.single.corr, fig.width=20, fig.height=12}
for (marker.type in unique(markers[,2])){
  marker.set <- which(rownames(sce_naive) %in% markers[markers[,2]==marker.type,1])
  name.pca <- paste("pca_", marker.type, sep = "")
  assign(name.pca, plotPCA(sce_naive, exprs_values="norm_exprs", colour_by = markers[markers[,2]==marker.type,1][1], 
    feature_set = marker.set) + fontsize)
    name.tsne <- paste("tsne_", marker.type, sep = "")
    assign(name.tsne, plotTSNE(sce_naive, exprs_values="norm_exprs", colour_by='CDK1',
    perplexity=5, rand_seed=100) + fontsize)
}
...
```{r naive.pca.single.corr2, fig.width=20, fig.height=12}
pc.out <- prcomp(t(assay(sce_naive, "norm_exprs")))
attr(pc.out$x, "percentVar") <- pc.out$sdev^2/sum(pc.out$sdev^2)
reducedDim(sce_naive, "PCA") <- pc.out$x
dim(reducedDim(sce_naive, "PCA"))

for (marker.type in unique(markers[,2])){
  marker <- which(rownames(sce_naive) %in% markers[markers[,2]==marker.type,1])
  pc.out <- prcomp(t(assay(sce_naive, "norm_exprs")[marker,,drop=FALSE]))
  attr(pc.out$x, "percentVar") <- pc.out$sdev^2/sum(pc.out$sdev^2)
  reducedDim(sce_naive, "PCA") <- pc.out$x
  nam <- paste("pca_", marker.type, sep = "")
  assign(nam, plotPCA(sce_naive, main = marker.type, colour_by = markers[markers[,2]==marker.type,1][1]) + fontsize)
}
...
collected <- list()
for (i in 1:4) {
    collected[[i]] <- plotExpression(sce_naive, x=vs.mod$gene1[i], 
...
collected <- list()
for (i in 1:4) {
    collected[[i]] <- plotExpression(sce_naive, x=vs.read$gene1[i], 
...
```{r}
ii <- 1
naive_mod <- data.frame(marker=unique(markers[,2]), total.markers = NA, total.corrs = NA, sig.corrs = NA)
...
for (marker.type in unique(markers[,2])){
  ...
  write.table(file=gzout, vs.mod, sep="\t", quote=FALSE, col.names=NA)
  close(gzout)
  naive_mod[ii,c(2,3,4)] <- c(length(chosen_markers), dim(vs.mod)[1], sig.count)
  ii <- ii+1
}
```
...
```{r}
ii <- 1
naive_read <- data.frame(marker=unique(markers[,2]), total.markers = NA, total.corrs = NA, sig.corrs = NA)
...
for (marker.type in unique(markers[,2])){
  ...
  write.table(file=gzout, vs.mod, sep="\t", quote=FALSE, col.names=NA)
  close(gzout)
  naive_read[ii,c(2,3,4)] <- c(length(chosen_markers), dim(vs.read)[1], sig.count)
  ii <- ii+1
}
```
