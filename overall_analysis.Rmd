...
We do this with an expression matrix in which the batch effect has been regressed out. 

```{r}
keep <- var.out$bio > 0
summary(keep)
batch.adjusted <- removeBatchEffect(logcounts(sce), sce$batch)
pc.out <- prcomp(t(batch.adjusted[keep,,drop=FALSE]), rank=chosen.npcs)
