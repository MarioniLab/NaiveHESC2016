...
for (b in levels(sce$batch)) {
    ...
    plot(dec.out$mean, dec.out$total, pch=16, col="grey", xlab="Mean log-expression",
...
These differences reflect the variation in technical noise in each data set, which affects the biological resolution of the analysis.
...
keep <- var.out$bio > 0
summary(keep)
pc.out <- prcomp(t(assay(sce, "norm_exprs")[keep,,drop=FALSE]), rank=chosen.npcs)
