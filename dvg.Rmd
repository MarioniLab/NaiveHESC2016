...
We load in the counts for each cell.
We identify the transition cells in the naive population to prune them out.
This ensures that we're estimating the dispersion for reasonably homogeneous populations.
...
We need to recalculate the average counts for the remaining cells.
(This also accounts for size factors for each class of features that wasn't considered in the existing `AveCounts`.)

```{r}
library(scater)
rowData(sce)$AveCount <- calcAverage(sce)
summary(rowData(sce)$AveCount)
```
...
We estimate the dispersion for each sample.
This gives us biological components after we take the residual from the technical trend.

```{r, fig.height=10, fig.width=10}
source("testing/varfun.R")
...
```{r}
keep <- rowMeans(bio.mat) > 0 & rowData(sce)$AveCount > 5
vals <- vals[keep,]
summary(keep)
``` 
