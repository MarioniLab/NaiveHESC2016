...
markers <- read.csv("miscellaneous/ferd_marker_set.csv", stringsAsFactors = FALSE)
...
First we focus on the genes that are differentially expressed between naive and primed. We check whether they are in the list of DE genes and if yes, whether they are signifcantly different.

@Aaron&Ferdinand:
Could we test for cell cycle related genes here as well? 
...
data.frame(primed_up = dim(primed_up_endo)[1], primed_down = dim(primed_down_endo)[1], not_significant = sum(DE_endo$FDR > 0.05), row.names = 'gene count')
...
data.frame(primed_up = dim(primed_up_ecto)[1], primed_down = dim(primed_down_ecto)[1], not_significant = sum(DE_ecto$FDR > 0.05), row.names = 'gene count')
...
data.frame(primed_up = dim(primed_up_meso)[1], primed_down = dim(primed_down_meso)[1], not_significant = sum(DE_meso$FDR > 0.05) , row.names = 'gene count')
```
...
print('primed hvg vs endoderm')
length(endo_markers)
sum(rownames(primed.hvg) %in% rownames(sce)[endo_markers])
primed.hvg_endo <- primed.hvg[which(rownames(primed.hvg) %in% rownames(sce)[endo_markers]),]


print('primed hvg vs ectoderm')
length(ecto_markers)
sum(rownames(primed.hvg) %in% rownames(sce)[ecto_markers])
primed.hvg_ecto <- primed.hvg[which(rownames(primed.hvg) %in% rownames(sce)[ecto_markers]),]


print('primed hvg vs mesoderm')
length(meso_markers)
sum(rownames(naive.hvg) %in% rownames(sce)[meso_markers])
