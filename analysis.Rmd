...
#set date:
date <- "2016_12_06"
dir.create(file.path("~/Documents/vMeyenn/paper_files/output", date), showWarnings = FALSE)
output <- file.path("~/Documents/vMeyenn/paper_files/output", date)
setwd(file.path("~/Documents/vMeyenn/paper_files/output", date))
```
...
for (sample in c("2383", "2384", "2677", "2678", "2739", "2740")){  
  qual_control <- read.table(file = paste("~/Documents/vMeyenn/data/my_qual_", sample, ".tsv", sep = ""), 
...
for (sample in c("2383", "2384", "2677", "2678", "2739", "2740")){
  path <- paste("~/Documents/vMeyenn/data/genic_counts_", sample, ".tsv", sep = "")
...
saveRDS(ensemblGenes, file = "~/Documents/vMeyenn/paper_files/objects/ensemblGenes")
...
dim(sce)
saveRDS(sce, file="~/Documents/vMeyenn/paper_files/objects/sce_new_raw")
```
...
saveRDS(sce, file = "~/Documents/vMeyenn/paper_files/objects/sce_new_object")
...
gz_all <- gzfile("./var.cor.gz", "w")
write.table(var.cor, gz_all, sep="\t", quote=FALSE, row.names=FALSE) 
close(gz_all)
head(var.cor)
dim(var.cor)
```
...
saveRDS(chosen, file = "~/Documents/vMeyenn/paper_files/objects/chosen1")
...
saveRDS(res_all, file = "~/Documents/vMeyenn/paper_files/objects/res_all")
```
...
saveRDS(is.de_all, file = "~/Documents/vMeyenn/paper_files/objects/is.de_all")
summary(is.de_all)
```
...
saveRDS(hvg.out_naive, file = "~/Documents/vMeyenn/paper_files/objects/hvg.naive")
head(hvg.out_naive)
```
...
gz_naive <- gzfile("var.cor_naive.gz", "w")
write.table(var.cor_naive, gz_naive, sep="\t", quote=FALSE, row.names=FALSE) 
close(gz_naive)
head(var.cor_naive)
dim(var.cor_naive)
```
...
saveRDS(sce_trans, file = "~/Documents/vMeyenn/paper_files/objects/new_sce_trans")
...
saveRDS(re.pheno, file = "~/Documents/vMeyenn/paper_files/objects/pheno")
heat.vals <- top.exprs - rowMeans(top.exprs)
saveRDS(heat.vals, file = "~/Documents/vMeyenn/paper_files/objects/heat.vals")
...
write.table(file=paste0(output, "/Goana.tsv"), go, sep="\t", quote=FALSE, col.names = NA) 
topGO(go, n=15, ontology = "BP") 
# KEGG 
kegg <- kegga(rownames(DEmarker.entrez), species = "Hs")
write.table(file=paste0(output, "/KEGG.tsv"), kegg, sep="\t", quote=FALSE, col.names = NA) 
topKEGG(kegg, n=15)
```
...
write.table(file=paste0(output, "/Goana_naive_g1.tsv"), go_naive_g1, sep="\t", quote=FALSE, col.names = NA) 
topGO(go_naive_g1, n=15, ontology = "BP") 
# KEGG 
kegg_naive_g1 <- kegga(rownames(hvg_naive_entrez_g1), species = "Hs")
write.table(file=paste0(output, "/KEGG_naive_g1.tsv"), kegg_naive_g1, sep="\t", quote=FALSE, col.names = NA) 
topKEGG(kegg_naive_g1, n=15)
```
...
saveRDS(hvg.out_primed, file = "~/Documents/vMeyenn/paper_files/objects/hvg.primed")
head(hvg.out_primed)
```
...
gz_primed <- gzfile("var.cor_primed.gz", "w")
write.table(var.cor_primed, gz_primed, sep="\t", quote=FALSE, row.names=FALSE) 
close(gz_primed)
head(var.cor_primed)
dim(var.cor_primed)
```
...
saveRDS(hvg.matrix,file = "~/Documents/vMeyenn/paper_files/objects/hvg.matrix")
...
plot(hvg.out_naive[shared_genes,]$bio, hvg.out_primed[shared_genes,]$bio, xlim=c(0,8), ylim=c(0,8), 
     pch = 19, cex = 0.5, main = "Gene expression of shared HVG genes",  ylab = "Biological Variability of Primed Subpopulation", 
...
saveRDS(all_markers, file = "~/Documents/vMeyenn/paper_files/objects/markers.map")
saveRDS(naive_markers, file = "~/Documents/vMeyenn/paper_files/objects/n.markers.map")
saveRDS(primed_markers, file = "~/Documents/vMeyenn/paper_files/objects/p.markers.map")
```
...
```{r, map2}
count_data <- read.table("~/Documents/vMeyenn/petropoulos/counts.txt")
...
saveRDS(stages, file = "~/Documents/vMeyenn/paper_files/objects/stages.petro")
saveRDS(nums, file = "~/Documents/vMeyenn/paper_files/objects/nums.petro")
...
```{r, mousecounts}
mouse_counts <- raw_mouse_counts <- read.table("~/Documents/vMeyenn/mouse/RawCounts_CellsFilt.txt", sep = "\t", header = TRUE)
...
saveRDS(stages, file = "~/Documents/vMeyenn/paper_files/objects/stages.mouse")
saveRDS(nums, file = "~/Documents/vMeyenn/paper_files/objects/nums.mouse")
...
```{r}
monkey_counts  <- raw_monkey_counts <- read.table("~/Documents/vMeyenn/monkey/SC3seq_Cy_ProcessedData.txt", sep = "\t", header = TRUE)
...
saveRDS(stages, file = "~/Documents/vMeyenn/paper_files/objects/stages.monkey")
saveRDS(nums, file = "~/Documents/vMeyenn/paper_files/objects/nums.monkey")
...
```{r}
sessionInfo()
saveRDS(sce, file="~/Documents/vMeyenn/paper_files/objects/new_sce_all_object")
saveRDS(sce_primed, file="~/Documents/vMeyenn/paper_files/objects/new_sce_primed_object")
saveRDS(sce_naive, file="~/Documents/vMeyenn/paper_files/objects/new_sce_naive_object")
```
