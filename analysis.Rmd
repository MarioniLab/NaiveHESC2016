...
# Set 'working directory' and 'output directory'. Define input-data directory.
...
saveRDS(res_all, file = paste0(objectpath, "res_all"))
```
...
saveRDS(var.out_naive, file = paste0(objectpath, "var.naive"))
...
e.y.vs.n <- exprs(sce)[!isSpike(sce),all.pheno!="primed"]
batch.vs.n <- sce[,!all.pheno=="primed"]$batch
design.vs.n <- model.matrix(~0 + naive.pheno + batch.vs.n)
fit <- lmFit(e.y.vs.n, design.vs.n)
con <- makeContrasts(naive.phenotransition - naive.phenonaive, levels=design.vs.n)
fit2 <- contrasts.fit(fit, con)
fit2 <- eBayes(fit2, robust=TRUE, trend=TRUE)
res <- topTable(fit2, n=Inf, sort.by="p")

marker_nt <- cbind(res$logFC, res$P.Value)
rownames(marker_nt) <- rownames(res)
...
Primed vs Trans
```{r}
primed.pheno <- factor(all.pheno[!all.pheno=="naive"], levels=c("primed", "transition"))
e.y.vs.p <- exprs(sce)[!isSpike(sce),all.pheno!="naive"]
batch.vs.p <- sce[,!all.pheno=="naive"]$batch
design.vs.p <- model.matrix(~0 + primed.pheno + batch.vs.p)
fit <- lmFit(e.y.vs.p, design.vs.p)
con <- makeContrasts(primed.phenotransition - primed.phenoprimed, levels=design.vs.p)
fit2 <- contrasts.fit(fit, con)
fit2 <- eBayes(fit2, robust=TRUE, trend=TRUE)
res <- topTable(fit2, n=Inf, sort.by="p")

marker_tp <- cbind(res$logFC, res$P.Value)
rownames(marker_tp) <- rownames(res)
...
```{r, heatNvPvT, fig.cap = "**Figure 27:** Heatmap of DE-expressed genes of transition subpopulation against naive and primed subpopulation, respectively.", fig.height=30, fig.width=10}
markerset.nt <- marker_nt[order(marker_nt[1:100,"logFC"], decreasing = TRUE),]
markerset.tp <- marker_tp[order(marker_tp[1:100,"logFC"], decreasing = TRUE),]
...
comparedDE <- cbind(NvsP, NvsT, PvsT)[,c(1,6,8)]
...
tsne1_naive_g1 <- plotTSNE(sce_naive_g1, exprs_values="norm_exprs", colour_by=top.hvg_naive_g1,
                  perplexity=5, rand_seed=150, feature_set=chosen_naive_set_g1) + fontsize
...
saveRDS(var.out_primed, file = paste0(objectpath, "var.primed"))
...
Variability might be driven by logFC only. 
```{r, fig.cap="**Figure 50:** Change in biological variability between naive and primed cells against logFC"}
...
```{r, fig.cap="**Figure 51:** Distribution of size factors in naive and primed subpopulation."}
...
```{r, MA, fig.cap="**Figure 52:** Ratio of assigned naive and assigned primed cells in the vonMeyenn data set. Abline indicates 0-hypothesis in which both primed and expressed naive genes are expressed equally."}
threshold <- 10
par(mfrow=c(2,2))
...
```{r, MA2b, fig.cap="**Figure 53:** Ratio of assigned naive and assigned primed cells in the Petropoulos data set. Abline indicates a state of the cell in which both expressed primed and expressed naive genes are the same.", fig.height = 14}
par(mfrow=c(4,2))
...
```{r, MA3b, fig.cap="**Figure 54:** Ratio of identified naive and identified primed cells of the Wolf MM ESCs for each time stage.", fig.height=11, fig.width=10}
for (cell in colnames(exprs_marked)){ 
  naive_genes <- exprs_marked[which(exprs_marked[rownames(naive_markers),cell] > threshold), cell]
  primed_genes <- exprs_marked[which(exprs_marked[rownames(primed_markers),cell] > threshold), cell]
...
```{r MA4, fig.cap="**Figure 55:** Ratio of identified naive and identified primed cells of monkey ESCs for each time stage.", fig.height=11, fig.width=14}
for (cell in colnames(exprs_marked)){ 
  naive_genes <- exprs_marked[which(exprs_marked[rownames(naive_markers),cell] > threshold), cell]
  primed_genes <- exprs_marked[which(exprs_marked[rownames(primed_markers),cell] > threshold), cell]
...
Read in all SNP calls and calculate proportions for heterozygous SNPs on X and for heterzugous SNP calls on X.
...
all.props <- list()
for (sample in c("2383", "2384", "2677", "2678", "2739", "2740")){
  path <- paste0(input, "xchrom/real-", sample, "_20161024/")
  current.props <- list()
  for (file in list.files(path = path, pattern = "*.vcf.gz", full.names = T)){
  
  blah <- readVcf(file)
  is.x <- as.logical(seqnames(blah)=="chrX")
  is.het <- geno(blah)$GT=="0/1"
  het <- sum(is.het & is.x)/sum(is.het) # Proportion of hets on X
  snpcalls <- sum(is.het & is.x)/sum(is.x) # Proportion of X SNP calls that are het
  current.props[[file]] <- c(het = het, snpcalls = snpcalls)
  } 
  
  current.props <- as.data.frame(current.props)
  cellname <- colnames(current.props)
  cellname <- gsub("^.*?_36", "36", cellname)
  cellname <- gsub("_L00._.*$", "", cellname)
  colnames(current.props) <- paste0(cellname, "_" , sample)
  current.props <- sumTechReps(current.props)
    
  all.props[[sample]] <- current.props
}

combined.props <- do.call(cbind, all.props)

# Take technical replicates and merge them
prop.names <- colnames(combined.props)
colnames(combined.props) <- sub("(.*)_2.*", "\\1", prop.names)
combined.props <- sumTechReps(combined.props)
colnames(combined.props) <- prop.names[match(colnames(combined.props), gsub("(.*)_2.*", "\\1", prop.names))]
dim(combined.props)
```
...
```{r}
sample <- sub(".*_([0-9]+)$", "\\1", colnames(combined.props))
naive.props <- combined.props[,sample %in% c("2383", "2677", "2678")]
primed.props <- combined.props[,sample %in% c("2384", "2739", "2740")]
```
...
for (sample in c("2383", "2384", "2677", "2678", "2739", "2740")){
  ...
  blah <- readVcf(file)
  is.auto <- as.logical(!seqnames(blah)=="chrX")
  is.het <- geno(blah)$GT=="0/1"
  het <- sum(is.het & is.auto)/sum(is.het) # Proportion of hets on autosomal chromsomes
  snpcalls <- sum(is.het & is.auto)/sum(is.auto) # Proportion of autosomal SNP calls that are het
  current.props[[file]] <- c(het = het, snpcalls = snpcalls)
  } 
  ...
  new.props[[sample]] <- current.props
}

autosome.props <- do.call(cbind, new.props)

# Take technical replicates and merge them
auto.names <- colnames(autosome.props)
colnames(autosome.props) <- sub("(.*)_2.*", "\\1", auto.names)
autosome.props <- sumTechReps(autosome.props)
colnames(autosome.props) <- prop.names[match(colnames(autosome.props), gsub("(.*)_2.*", "\\1", auto.names))]
dim(autosome.props)
```

```{r, fig.cap="**Figure 56:** Proportions of heterozygous SNPs on X-Chrom vs Autosome (top) and proportion of heterozygocity  only on X chromosome (bottom)."}
sample <- sub(".*_([0-9]+)$", "\\1", colnames(combined.props))
naive.auto <- autosome.props[,sample %in% c("2383", "2677", "2678")]
primed.auto <- autosome.props[,sample %in% c("2384", "2739", "2740")]

par(mfrow=c(2,2))
plot(density(naive.props["het",], na.rm = T), main = "Allosome", xlim = c(0, 0.1), 
     xlab = "Proportion of hets on Xchrom vs hets on autosome", col = "#e79f00")
mtext(text = paste0("mean naive: ", round(mean(naive.props["het",], na.rm = T), digits = 3), " | mean primed: ", 
                    round(mean(primed.props["het",], na.rm = T), digits = 3)), c(3))
lines(density(primed.props["het",], na.rm = T), col = "#0072B2")
legend("topright", legend = c("Primed", "Naive"), pch=16,  col = c("#0072B2", "#e79f00"), bty = "n")

plot(density(naive.props["snpcalls",], na.rm = T), main = "Allosome", xlab = "Proportion of hets on X", 
     xlim = c(0, 0.13), col = "#e79f00", ylim = c(0,20))
mtext(text = paste0("mean naive: ", round(mean(naive.props["snpcalls",], na.rm = T), digits = 3), 
                    " | mean primed: ", round(mean(primed.props["snpcalls",], na.rm = T), digits = 3)), c(3))
lines(density(primed.props["snpcalls",], na.rm = T), col = "#0072B2")
legend("topright", legend = c("Primed", "Naive"), pch=16,  col = c("#0072B2", "#e79f00"), bty = "n")

plot(density(naive.auto["het",], na.rm = T), main = "Autosome", 
     xlab = "Proportion of hets on autosomal vs hets on X", col = "#e79f00")
mtext(text = paste0("mean naive: ", round(mean(naive.auto["het",], na.rm = T), digits = 3), 
                    " | mean primed: ", round(mean(primed.auto["het",], na.rm = T), digits = 3)), c(3))
lines(density(primed.auto["het",], na.rm = T), col = "#0072B2")
legend("topright", legend = c("Primed", "Naive"), pch=16,  col = c("#0072B2", "#e79f00"), bty = "n")

plot(density(naive.auto["snpcalls",], na.rm = T), main = "Autosome", 
     xlab = "Proportion of hets on autosomal chromosomes", col = "#e79f00", ylim = c(0,15))
mtext(text = paste0("mean naive: ", round(mean(naive.auto["snpcalls",], na.rm = T), digits = 3), 
                    " | mean primed: ", round(mean(primed.auto["snpcalls",], na.rm = T), digits = 3)), c(3))
lines(density(primed.auto["snpcalls",], na.rm = T), col = "#0072B2")
legend("topright", legend = c("Primed", "Naive"), pch=16,  col = c("#0072B2", "#e79f00"), bty = "n")
```
