...
collected.npcs_naive <- denoisePCA(sce_naive, var.out_naive, value="n", assay.type="norm_exprs")
```
...
summary(keep)
pc.out <- prcomp(t(assay(sce_naive, "norm_exprs")[keep,,drop=FALSE]), rank=collected.npcs_naive)
...
sce_naive <- runTSNE(sce_naive, use_dimred="PCA", perplexity=10, set.seed=100)
...
```{r naive_PCA}
my.clusters_naive <- unname(cutree(my.tree_naive, k=3))er
sce_naive$cluster <- factor(my.clusters_naive)
plotPCA(sce_naive, colour_by="cluster") + fontsize
```
