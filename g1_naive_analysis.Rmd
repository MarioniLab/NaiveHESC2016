---
title: "Single-cell RNA-seq of naive/primed embryonic stem cells: G1-only, naive-only analysis"
...
dir.create("figure-g1_naive/", showWarning=FALSE)
knitr::opts_chunk$set(error=FALSE, warning=FALSE, message=FALSE, fig.path="figure-g1_naive/")
...
lines(var.out_naive$mean[o], var.out_naive$tech[o], col="dodgerblue", lwd=2)
```
...
With these clusters identified, we can assess DE of cluster 3 relative to each other cluster. 
...
clust.num_naive <- as.character(trans.cluster)
...
o <- order(all.pheno)
top.exprs <- norm_exprs(sce)[fData(sce)$ensembl_gene_id %in% imprinted,o,drop=FALSE]
re.pheno <- factor(all.pheno[o])
clust.cols <- c("black", "grey", "red")
heat.vals <- top.exprs - rowMeans(top.exprs)
heatmap.2(heat.vals, col=bluered, symbreak=TRUE, trace='none', cexRow=0.6, dendrogram='row',
