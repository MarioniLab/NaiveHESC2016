...
```{r naive.varplot}
write.table(file=file.path(resdir, "var_g1_naive.tsv"), var.out_naive, sep="\t", quote=FALSE, col.names=NA)
```
...
sce_naive <- runTSNE(sce_naive, exprs_values="norm_exprs", perplexity=5, set.seed=100, feature_set=chosen_naive)
...
sce_naive <- runPCA(sce_naive, exprs_values="norm_exprs", perplexity=10, set.seed=100, feature_set = chosen_naive)
...
These suggest that `k=3` is the best, with cluster 2 being genuinely separated from the others.
...
sce <- runPCA(sce, exprs_values="norm_exprs")
