...
First, we first remove the batch effect in each population separately to ensure that they don't distord the clustering. 
...
saveRDS(pca_naive, file = 'naive_plot.Rds')
saveRDS(pca_primed, file = 'primed_plot.Rds')
```
...
```{r}
pca_naive <- plotPCA(sce_naive, exprs_values="norm_exprs", colour_by = 'CDK1', feature_set = lin.markers, rerun=TRUE) + fontsize
tsne_naive <- plotTSNE(sce_naive, exprs_values="norm_exprs", colour_by= "CDK1", feature_set =  lin.markers, rerun=TRUE, perplexity=5, rand_seed=100) + fontsize
...
```{r}
#mesoderm
pca_naive <- plotPCA(sce_naive, exprs_values="norm_exprs", colour_by = 'SNAI1', feature_set = lin.markers, rerun=TRUE) + fontsize
...
#ectoderm
pca_naive <- plotPCA(sce_naive, exprs_values="norm_exprs", colour_by = 'NES', feature_set = lin.markers, rerun=TRUE) + fontsize
...
tsne_naive <- plotTSNE(sce_naive, exprs_values="norm_exprs", colour_by= "GATA4", feature_set =  lin.markers, rerun=TRUE, perplexity=5, rand_seed=100) + fontsize
multiplot(pca_naive, tsne_naive)
...
#mesoderm
pca_primed <- plotPCA(sce_primed, exprs_values="norm_exprs", colour_by = 'SNAI1', feature_set = lin.markers, rerun=TRUE) + fontsize
...
#ectoderm
pca_primed <- plotPCA(sce_primed, exprs_values="norm_exprs", colour_by = 'NES', feature_set = lin.markers, rerun=TRUE) + fontsize
...
tsne_primed <- plotTSNE(sce_primed, exprs_values="norm_exprs", colour_by= "GATA4", feature_set =  lin.markers, rerun=TRUE, perplexity=5, rand_seed=100) + fontsize
multiplot(pca_primed, tsne_primed)
...
For this, we first have to loop through all the lists, extracting the correlating genes and putting them into a matrix that uses respective lineage markers as row and epigenetic readers/writers as columns. 

```{r}
all.heats <- data.frame()
for (marker.type in unique(markers[,2])){
  current_list <- naive_mod[[marker.type]]
  current_list <- current_list[1:206,]
  ...
  epi.names <- unique(c(current_list$gene1[current_list$gene1 %in% rownames(sce[epi_modifier,])], 
                           current_list$gene2[current_list$gene2 %in% rownames(sce[epi_modifier,])]))
...
naive.mod.heat <- all.heats
```

Of course, we have to repeat this for naive/readers primed/mod and primed/readers.

```{r}
all.heats <- data.frame()
for (marker.type in unique(markers[,2])){
  current_list <-naive_read[[marker.type]] 
  current_list <- current_list[1:362,]
  current.markers <- rownames(sce[rownames(sce) %in% markers[markers[,2]==marker.type,1],])
  
  lineage.names <- unique(c(current_list$gene1[current_list$gene1 %in% current.markers], 
                          current_list$gene2[current_list$gene2 %in% current.markers]))
  epi.names <- unique(c(current_list$gene1[current_list$gene1 %in% rownames(sce[epi_reader,])], 
...
all.heats <- data.frame()
for (marker.type in unique(markers[,2])){
  current_list <-primed_mod[[marker.type]] 
  current_list <- current_list[1:243,]
  current.markers <- rownames(sce[rownames(sce) %in% markers[markers[,2]==marker.type,1],])
...
primed.mod.heat <- all.heats[,1:25]
```
...
for (marker.type in unique(markers[,2])){
  ...
  current_list <- current_list[1:584,]
  current.markers <- rownames(sce[rownames(sce) %in% markers[markers[,2]==marker.type,1],])
...
primed.read.heat <- all.heats
```

Finally, this allows us to create the heatmaps and saving the objects.

```{r}
pheatmap(naive.read.heat)
pheatmap(naive.mod.heat)
pheatmap(primed.mod.heat)
pheatmap(primed.read.heat)

saveRDS(naive.read.heat, 'naive.read.Rds')
saveRDS(naive.mod.heat, 'naive.mod.Rds')
saveRDS(primed.mod.heat,'primed.mod.Rds')
saveRDS(primed.read.heat,'primed.read.Rds')
```
...
all.heats <- data.frame()
for (marker.type in unique(markers[,2])){
  current_list <-naive_epi[[marker.type]] 
  current_list <- current_list[1:506,]
  current.markers <- rownames(sce[rownames(sce) %in% markers[markers[,2]==marker.type,1],])
...
for (marker.type in unique(markers[,2])){
  ...
  current_list <- current_list[1:784,]
  current.markers <- rownames(sce[rownames(sce) %in% markers[markers[,2]==marker.type,1],])
...
primed_epi.heat <- all.heats
```

Plot and save objects

```{r}
pheatmap(naive_epi.heat)
pheatmap(primed_epi.heat)

saveRDS(naive_epi.heat, 'naive.epi.Rds')
saveRDS(primed_epi.heat, 'primed.epi.Rds')
```
