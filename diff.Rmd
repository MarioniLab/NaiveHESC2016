...
for (marker.type in unique(markers[,2])){
  ...
  current_list <- current_list[1:205,]
...
for (marker.type in unique(markers[,2])){
  ...
  current_list <- current_list[1:413,]
  current.markers <- rownames(sce[rownames(sce) %in% markers[markers[,2]==marker.type,1],])
...
for (marker.type in unique(markers[,2])){
  ...
  current_list <- current_list[1:225,]
  current.markers <- rownames(sce[rownames(sce) %in% markers[markers[,2]==marker.type,1],])
...
Finally, this allows us to create the heatmaps.
...
Saving the objects and printing session information.
