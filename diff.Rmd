...
for (marker.type in unique(markers[,2])){
  ...
  naive_mod <- rbind(naive_mod, vs.read)#
...
for (marker.type in unique(markers[,2])){
  ...
  naive_read <- rbind(naive_read, vs.read)#
...
for (marker.type in unique(markers[,2])){
  ...
  primed_mod <- rbind(primed_mod, vs.mod)#
...
for (marker.type in unique(markers[,2])){
  ...
  primed_read <- rbind(primed_read, vs.read)
...
lineage.names <- unique(c(primed_read$gene1[primed_read$gene1 %in% lin.markers], 
                          primed_read$gene2[primed_read$gene2 %in% lin.markers]))
epi.read.names <- unique(c(primed_read$gene1[primed_read$gene1 %in% rownames(sce[epi_reader,])], 
                           primed_read$gene2[primed_read$gene2 %in% rownames(sce[epi_reader,])]))

heatmat <- matrix(nrow=length(lineage.names), ncol = length(epi.read.names))
rownames(heatmat) <- lineage.names
colnames(heatmat) <- epi.read.names

for (lin in lineage.names){
    for (epi in epi.read.names){
      if (any(primed_read$gene1 == lin & primed_read$gene2 == epi)){
        heatmat[lin, epi] <- primed_read$rho[which(primed_read$gene1 == lin & primed_read$gene2 == epi)]
      }
      else if (any(primed_read$gene2 == lin & primed_read$gene1 == epi)){
        heatmat[lin, epi] <- primed_read$rho[which(primed_read$gene1 == lin & primed_read$gene2 == epi)]
      }
  else {heatmat[lin, epi] <- NA}
}}
