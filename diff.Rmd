...
We can also colour the plots again for cell-cycle expression, which shows that even when separating by lineage markers, cell cycle still seems to be the main influence in variability. 

```{r}
pca_primed <- plotPCA(sce_primed, exprs_values="norm_exprs", colour_by = 'CDK1', feature_set = lin.markers, rerun=T) + fontsize
tsne_primed <- plotTSNE(sce_primed, exprs_values="norm_exprs", colour_by= "CDK1", feature_set =  lin.markers, rerun=T, perplexity=5, rand_seed=100) + fontsize
...
We can now visualize these 4 lists with a heatmap.

```{r}
library(pheatmap)
epi_mat <- cbind(naive_mod_stat$sig.corrs, naive_read_stat$sig.corrs, 
               primed_mod_stat$sig.corrs, primed_read_stat$sig.corrs)
rownames(epi_mat) <- naive_mod_stat$marker
colnames(epi_mat) <- c('naive mod', 'naive read', 'primed mod', 'primed read')
pheatmap(epi_mat, cluster_row = F, cluster_cols = F)
```
