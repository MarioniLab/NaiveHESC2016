...
sce_primed <- runTSNE(sce_primed, exprs_values="norm_exprs", perplexity=5, set.seed=100, feature_set=chosen_primed)
...
sce_primed <- runPCA(sce_primed, exprs_values="norm_exprs", perplexity=10, set.seed=100, feature_set = chosen_primed)
...
```{r}
var.out_primed <- read.table(file.path("results-primed", "var_primed.tsv"))
hvg.out_naive <- read.table(file.path("results-naive", "var_naive.tsv"))
hvg.out_primed <- var.out_naive$bio < 
hvg.out_naive
nrow(hvg.out_primed)
nrow(hvg.out_naive)
plot(hvg.out_primed$mean, hvg.out_primed$tech, col="dodgerblue", pch=16, cex=0.5,
    xlab="Mean log-expression", ylab="Technical variance")
points(hvg.out_naive$mean, hvg.out_naive$tech, col="orange", pch=16, cex=0.5)
```
...
dbio <- hvg.out_primed$bio - hvg.out_naive$bio
