...
for (b in levels(sce_primed$batch)) {
    ...
    var.out <- decomposeVar(cur.sce, var.fit, assay="exprs")
...
lines(var.out_primed$mean[o], var.out_primed$tech[o], col="dodgerblue", lwd=2)
```
...
for (entry in names(all.sets)) {
    m <- which(fData(sce_primed)$entrezgene %in% all.sets[[entry]])
