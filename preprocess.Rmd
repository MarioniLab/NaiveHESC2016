...
for (sample in c("2383", "2384", "2677", "2678", "2739", "2740")){  
    cur.file <- list.files("./Raw Files", pattern=paste0("my_qual_", sample), full=TRUE)
...
gene.length <- NULL
for (sample in c("2383", "2384", "2677", "2678", "2739", "2740")){
    cur.file <- list.files("./Raw Files", pattern=paste0("genic_counts_", sample), full=TRUE)
...
## Constructing the `SingleCellExperiment Set (SCESet)` object
...
for (b in levels(sce$batch)) { 
    ...
    mito.drop <- isOutlier(sce$pct_counts_Mt[cur.batch], nmads=3, type="higher")
    spike.drop <- isOutlier(sce$pct_counts_ERCC[cur.batch], nmads=3, type="higher")
