...
ensembl <- useMart(biomart='ENSEMBL_MART_ENSEMBL', dataset="hsapiens_gene_ensembl", 
    host="dec2015.archive.ensembl.org") # Ensembl version 83.
...
multiplot(cols=2, 
    plotPhenoData(sce, aes(x = sample, y = total_counts, colour_by = phenotype)),
    plotPhenoData(sce, aes(x = sample, y = total_features, colour_by = phenotype)))
```
...
multiplot(cols=2, 
    plotPhenoData(sce, aes(x=sample, y=pct_counts_feature_controls_Mt, colour_by = phenotype)),
    plotPhenoData(sce, aes(x=sample, y=pct_counts_feature_controls_Spike, colour_by = phenotype)))
```
...
```{r}
library(limma)
norm_exprs(sce) <- removeBatchEffect(exprs(sce), sce$batch)
plotExplanatoryVariables(sce, variables=c("counts_feature_controls_Spike",
    "log10_counts_feature_controls_Spike"), exprs_values="norm_exprs") + fontsize
```
